...
class TwigComponentConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
     ...
     */
    public function defaults(string $namespace, mixed $value = []): \Symfony\Config\TwigComponent\DefaultsConfig|static
    {
        if (!\is_array($value)) {
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('defaults', $value)) {
            $this->_usedProperties['defaults'] = true;
            $this->defaults = array_map(function ($v) { return \is_array($v) ? new \Symfony\Config\TwigComponent\DefaultsConfig($v) : $v; }, $value['defaults']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['defaults'])) {
            $output['defaults'] = array_map(function ($v) { return $v instanceof \Symfony\Config\TwigComponent\DefaultsConfig ? $v->toArray() : $v; }, $this->defaults);
        }
